...
import { Leaf, Award, Activity, Zap, Wifi, WifiOff, ArrowUpRight, History } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Listen for live point updates pushed from the Admin/AI backend
    socket.on('reward_issued', (data) => {
      // 1. Update Global Points
      updatePoints(data.newTotal);
      
      // 2. Add to local activity feed dynamically
      setRecentActivity(prev => [
        { id: Date.now(), title: 'Live: Report Verified!', points: `+${data.pointsEarned}`, time: 'Just now', status: 'verified' },
        ...prev.slice(0, 3) // Keep only the latest 4 items
      ]);
  ...
  return (
    <div className="max-w-7xl mx-auto pt-8 px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="absolute top-0 left-10 w-96 h-96 bg-green-500/10 rounded-full blur-[100px] pointer-events-none -z-10"></div>
      <div className="absolute top-40 right-10 w-72 h-72 bg-blue-500/10 rounded-full blur-[100px] pointer-events-none -z-10"></div>

      <motion.div variants={containerVars} initial="hidden" animate="show" className="space-y-8">
        
        {/* Header Area */}
        <motion.header variants={itemVars} className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 pb-4 border-b border-gray-200 dark:border-gray-800">
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white mb-2">
              Command Center
            </h1>
            <p className="text-gray-500 font-medium">Welcome back, {user.name}. Your environmental impact is growing.</p>
          </div>
          
          {/* Live Server Status */}
          <div className={`flex items-center space-x-2 px-4 py-2 rounded-full font-bold backdrop-blur-md transition-colors shadow-sm ${
            isConnected ? 'bg-green-500/10 text-green-600 border border-green-500/20' : 'bg-red-500/10 text-red-600 border border-red-500/20'
          }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-6">
          ...
          {/* Hero Metric - Total Points (Spans 8 cols on large screens) */}
          <motion.div variants={itemVars} className="glass rounded-[2rem] p-8 md:col-span-6 lg:col-span-8 flex flex-col justify-between relative overflow-hidden group shadow-xl">
            ...
            <div className="z-10">
              ...
              {/* Point Counter with Pop Animation on Change */}
              <AnimatePresence mode="popLayout">
                <motion.h2 
                  key={user.points} 
                  initial={{ y: -20, opacity: 0, scale: 1.1, filter: "blur(4px)", color: '#22c55e' }}
                  animate={{ y: 0, opacity: 1, scale: 1, filter: "blur(0px)", color: '#16a34a' }} // Adjust to standard text color based on theme
          ...
          {/* Secondary Metric - Rank (Spans 4 cols) */}
          <motion.div variants={itemVars} className="glass rounded-[2rem] p-8 md:col-span-3 lg:col-span-4 flex flex-col justify-between hover:-translate-y-1 transition-transform duration-300 shadow-lg">
            <div>
          ...
          {/* Secondary Metric - Streak (Spans 4 cols) */}
          <motion.div variants={itemVars} className="glass rounded-[2rem] p-8 md:col-span-3 lg:col-span-4 flex flex-col justify-between hover:-translate-y-1 transition-transform duration-300 shadow-lg relative overflow-hidden">
          ...
          {/* Activity Feed (Spans 8 cols) */}
          <motion.div variants={itemVars} className="glass rounded-[2rem] p-8 md:col-span-6 lg:col-span-8 flex flex-col shadow-lg">
